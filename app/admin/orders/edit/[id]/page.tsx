...
export default function EditOrderPage({
  ...
  useEffect(() => {
    params.then((p) => setOrderId(decodeURIComponent(p.id)));
  }, [params]);
  ...
  const fetchOrderDetails = async (id: string) => {
    ...
    const { data: orderData, error } = await supabase
      .from('orders')
      .select('*, customers(id, full_name, phone_number), order_items(*)')
      .eq('id', id)
      .single();

    if (orderData) {
      setPhoneNumber(orderData.customers.phone_number);
      setFullName(orderData.customers.full_name);
      setCustomerId(orderData.customer_id);

      const items: OrderItemForm[] = orderData.order_items.map((i: any) => ({
        id: i.id,
      ...
      // Migrasi: jika status lama "Active", ubah ke "Diproses"
      ...
      setNotification({
        type: 'error',
        text: 'Gagal memuat Order: ' + error.message,
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!phoneNumber || !fullName || orderItems.length === 0 || !orderId) {
      setNotification({
        type: 'error',
    ...
    try {
      ...
      setNotification({
        type: 'error',
        text: 'Gagal update pesanan: ' + error.message,
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="max-w-4xl mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-[#8B5E3C] tracking-tight">
              Edit <span className="text-blue-400">Pesanan</span>
            </h1>
          </div>
        </div>
      </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section className="bg-white p-8 rounded-[2.5rem] border border-orange-100 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              ...
              {orderId && (
                <div className="bg-orange-50 text-orange-600 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border border-orange-200">
                  ID: {orderId}
                </div>
              )}
            </div>
          ...
          <section className="bg-white p-8 rounded-[2.5rem] border border-orange-100 shadow-sm">
            ...
            <div className="space-y-4">
              {orderItems.map((item, index) => {
                ...
                // ✅ Cek apakah format saat ini valid (untuk data lama yang mungkin mismatch)
                const isCurrentFormatValid =
                  !item.format ||
                  availableFormats.some(
                    (f) => f.type.toLowerCase() === item.format.toLowerCase()
                  );
                    ...
                    <div className="w-full md:w-48">
                      ...
                      >
                        {availableFormats.length === 0 ? (
                          ...
                          <>
                            {/* ✅ Jika format lama tidak ada di DB, tampilkan sebagai warning */}
                            {hasTitle &&
                              item.format &&
                              !isCurrentFormatValid && (
                                <option value={item.format} disabled>
                                  ⚠️ {item.format} (tidak ada di database)
                                </option>
                              )}
                      ...
                      {/* ✅ Warning jika format lama mismatch */}
                      {hasTitle && !isCurrentFormatValid && item.format && (
                        <p className="text-[10px] text-red-500 mt-1 font-bold">
                          ⚠️ Format &quot;{item.format}&quot; tidak ada di
                          database untuk buku ini. Silakan pilih ulang.
                        </p>
                      )}
                    </div>
          ...
          <div className="flex justify-end gap-4 mt-8">
            ...
            <button
              disabled={loading}
              type="submit"
              className="px-8 py-4 rounded-full font-bold bg-blue-500 hover:bg-blue-600 text-white shadow-md flex items-center gap-2 transition-all active:scale-95 disabled:opacity-60"
            >
              {loading ? (
                <Loader2 className="w-5 h-5 animate-spin" />
              ) : (
